...
| Background haze | `canvas#bgHaze` — domain-warped fbm noise, ~24fps, slow horizontal flow + breathing swell, `screen`-blended over the near-black grad. **Idle while the hero covers the viewport** (invisible there, and it would steal CPU from the video), on tab-hide, and on ribbon hover. Self-freezes on weak devices (times its first 60 frames); one static frame under reduced-motion. Mesh/dots/scanlines/grain/mark are static. (Replaced the 3 CSS `.blob`s + `drift`.) Reference: santionispirits.com. |
...
| Hero video | `#heroVideo` (`club-hero.mp4`, 900px, via `data-src-mp4`) plays muted/looped behind the wordmark. **The dark treatment (desaturate + darken + contrast) is baked into the file — no runtime `filter:`** (a full-viewport filter on a playing video re-composites every frame and stutters weak GPUs; reworked 2026-08-30 after "la vidéo coupe beaucoup"). On top: `opacity 0.86`, a static `::after` voile (radial vignette + linear fade to `--ink`), and a static soft-light grain `::before`. Content at `z-index: 2`. Pauses off-screen / tab-hidden; skipped on Save-Data / 2G; off under reduced-motion. The background haze also idles whenever the hero covers the viewport. |
